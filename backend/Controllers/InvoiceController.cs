...
namespace Backend.Controllers
{
    ...
    public class InvoiceController : ControllerBase
    {
        ...
        [
            ...
            new InvoicePaymentMethodOptionDto { Value = "TINK", Label = "Tink" },
        ...
        public async Task<ActionResult<InvoiceFormOptionsDto>> GetFormOptions()
        {
            ...
            var systemAccounts = await _systemAccountService.EnsureAccountsAsync(user.OfficeId.Value);
            ...
            return Ok(new InvoiceFormOptionsDto
            {
                ...
                SystemAccountNumbers = new Dictionary<string, int?>
                {
                    [SystemAccountService.ReceivableCash] = systemAccounts[SystemAccountService.ReceivableCash].AccountNr,
                    [SystemAccountService.ReceivableInvoice] = systemAccounts[SystemAccountService.ReceivableInvoice].AccountNr,
                },
                TinkEnabled = await _context.Offices
                    .AsNoTracking()
                    .Where(office => office.Id == user.OfficeId.Value)
                    .Select(office => office.TinkEnabled)
                    .FirstOrDefaultAsync(),
            });
        }
        ...
        public async Task<ActionResult<InvoiceDto>> GetNewInvoice([FromQuery] int? reservationId, [FromQuery] string? receiverTypeCode)
        {
            ...
            var invoice = new InvoiceDto
            {
                ...
                AccountNr = systemAccounts[SystemAccountService.ReceivableInvoice].AccountNr,
                AccountNrVat = systemAccounts[SystemAccountService.VatDomestic].AccountNr,
                NrOfInvoiceDays = 30,
                TermsOfPayment = "30",
        ...
        public async Task<ActionResult<Invoice>> PostInvoice([FromBody] InvoiceUpsertDto invoice)
        {
            try
            {
                ...
                await RecomputeReservationStatusesForInvoiceAsync(invoiceInDb.Id, user.OfficeId);
        ...
        public async Task<IActionResult> DeleteInvoice(int id)
        {
            ...
            var invoice = await _context.Invoices
                .FirstOrDefaultAsync(item => item.Id == id && item.OfficeId == user.OfficeId);
            if (invoice == null)
            {
                return NotFound(new { message = "Invoice not found" });
            }

            var highestInvoiceNr = await _context.Invoices
                .Where(item => item.OfficeId == user.OfficeId && item.InvoiceNr.HasValue)
                .MaxAsync(item => (int?)item.InvoiceNr);

            if (!invoice.InvoiceNr.HasValue || invoice.InvoiceNr != highestInvoiceNr)
            {
                return Conflict(new { message = "Endast fakturan med högst fakturanummer kan tas bort. Makulera fakturan i stället." });
            }

            var invoiceRows = await _context.InvoiceRows
                .Where(row => row.InvoiceId == invoice.Id)
                .ToListAsync();
            _context.InvoiceRows.RemoveRange(invoiceRows);
        ...
        /// <summary>Recomputes and persists StatusCode for every reservation touched by this invoice's rows.</summary>
        private async Task RecomputeReservationStatusesForInvoiceAsync(int invoiceId, int? officeId)
        {
            var reservationIds = await _context.InvoiceRows
                .Where(row => row.InvoiceId == invoiceId
                    && row.ReservationCalcItemId.HasValue
                    && row.ReservationCalcItem!.ReservationCalc!.ReservationId.HasValue)
                .Select(row => row.ReservationCalcItem!.ReservationCalc!.ReservationId!.Value)
                .Distinct()
                .ToListAsync();
            ...
            foreach (var reservationId in reservationIds)
            {
                ...
                reservationForStatus.StatusCode = ReservationStatusCodes.Compute(itemsForStatus, calcsForStatus, categoryNamesById);
            }
