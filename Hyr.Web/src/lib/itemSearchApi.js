...
export async function searchItems({ conditions = [], sorts, pageNumber = 1, pageSize = 100 } = {}) {
  ...
  return {
    items: rows.map((item) => ({
      ...
      nrOfItemsTotal: item?.nrOfItemsTotal ?? item?.NrOfItemsTotal ?? null,
      basePrice: item?.basePrice ?? item?.BasePrice ?? null,
      pricePerDay: item?.pricePerDay ?? item?.PricePerDay ?? null,
      pricePerWeek: item?.pricePerWeek ?? item?.PricePerWeek ?? null,
      note: item?.note,
    })),
