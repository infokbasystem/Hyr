...
export default function DateRangePicker({
  ...
  const handleDay = useCallback(date => {
    if (singleDate) {
      setStartDate(date); setEndDate(date); setSelecting(false); setHoverDate(null); setActiveKey(null);
      setCommitted({ start: date, end: date });
  ...
  }, [selecting, startDate, singleDate, onApply, closePopup]);
  ...
  const showSidebar = presets.length > 0 && !singleDate;
  ...
  return (
    ...
    >
      ...
      {!popupOnly && (
        ...
        >
          ...
          <span className={["mt-[2px]", hasCommitted ? "text-gray-800" : "text-gray-700"].join(" ")}>
            {hasCommitted
              ? (singleDate ? fmtShort(committed.start) : `${fmtShort(committed.start)} – ${fmtShort(committed.end)}`)
              : placeholder}
          </span>
          {hasCommitted && (
            <span
      ...
      {(popupOnly || open) && (
        <div
          className={[
            popupOnly ? "relative" : "absolute z-50 top-[calc(100%+6px)] left-0",
        ...
        >
          ...
          <div className="flex">
            {/* Sidebar */}
            {showSidebar && (
              <div className="w-54 shrink-0 border-r border-slate-200 py-3 flex flex-col gap-0.5 mt-10 mb-3 px-3">
                {showClearPreset && (
                  <button
