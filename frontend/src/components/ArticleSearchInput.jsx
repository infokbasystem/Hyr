...
const ArticleSearchInput = ({
    ...
    const handleInputKeyDown = (event) => {
        if (!showPopup || searchResults.length === 0 || isSearching) {
            return;
        }
        ...
        if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
            ...
            setHighlightedIndex((previousIndex) => {
                ...
                return Math.max(0, Math.min(searchResults.length - 1, previousIndex + direction));
            });
            return;
        }

        if (event.key === 'Enter' && highlightedIndex >= 0 && searchResults[highlightedIndex]) {
            event.preventDefault();
            handleArticleClick(searchResults[highlightedIndex]);
        }
    };
                        ...
                        {showLoadingIndicator && searchResults.length === 0 ? (
                            ...
                            <ul className={`transition-opacity duration-150 ${isSearching ? 'opacity-40 pointer-events-none' : ''}`}>
                                {searchResults.map((article, index) => (
                                    <li
                                        key={article.id}
                                        className={`px-3 py-2 cursor-pointer border-b border-gray-100 last:border-b-0 hover:bg-blue-50 ${index === highlightedIndex ? 'bg-blue-50' : ''}`}
                                        onMouseDown={(e) => {
                                            e.preventDefault();
