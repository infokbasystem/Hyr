...
const Reservation = () => {
    ...
    const appliedNewReservationPrefillRef = useRef(false);
    ...
    const hasReservationItems = (reservation?.reservationItems || []).length > 0;
    const canReturnItems = (reservation?.reservationItems || []).length > 0
        && (reservation?.reservationItems || []).every(item => Boolean(item?.actualFrom));
    const canCheckIn = canReturnItems
        && (reservation?.reservationItems || []).every(item => Boolean(item?.actualTo));
    ...
    const handleCheckaIn = () => {
        markStale();
        setMessages(prev => prev.filter(msg => msg.type !== 'success'));
        setReservation(prev => ({
            ...prev,
            reservationItems: (prev?.reservationItems || []).map(item => ({
                ...item,
                isCheckedIn: true,
            })),
        }));
    ...
    const newReservationPrefill = location.state?.newReservationPrefill ?? null;

    useEffect(() => {
        if (!newReservationPrefill?.vehicle || appliedNewReservationPrefillRef.current) {
            return;
        }

        if (initialLoadPending || routeReservationId !== 'new' || !reservation || reservation.id !== 0) {
            return;
        }

        appliedNewReservationPrefillRef.current = true;
        ...
        handleCarSearch({
            period: {
                from: parsePrefillDate(newReservationPrefill?.period?.from),
                to: parsePrefillDate(newReservationPrefill?.period?.to),
            },
            selectedCar: newReservationPrefill.vehicle,
        });
    }, [newReservationPrefill, initialLoadPending, routeReservationId, reservation]);
    ...
    const deleteReservation = async () => {
        ...
        setMessages([{ type: 'success', text: 'Bokningen borttagen' }]);
        setTimeout(() => {
            if (new URLSearchParams(location.search).get('newTab') === 'true') {
                window.close();
                return;
            }
        ...
        <div className="flex h-full min-h-full w-full flex-1 flex-col px-0 py-0 md:px-[clamp(4px,3vw,6vw)]">
            ...
            <div className='mt-1 flex min-h-0 flex-1 flex-col'>
                ...
                <div className="grid min-h-0 flex-1 gap-6 lg:grid-cols-[280px_minmax(0,1fr)] lg:gap-8 lg:items-start">
                        ...
                        <aside className="lg:sticky lg:top-[calc(52px+72px+1rem)] lg:z-10 lg:self-start">
                            <div className="space-y-4 pr-0 pb-4 ml-2">
                                ...
                                <div className="space-y-2 text-xs text-gray-500" style={{ fontFamily: "'Neue Haas Unica', 'Helvetica Neue', Arial, sans-serif" }}>
                                    ...
                                    {renderMetaRow('Ändrad:', reservation?.modifiedDate, formatUserName(reservation?.modifiedByUserName))}
                                </div>
                            </div>
                    ...
                    <div className='flex-grow lg:pl-2 pb-10 mr-20'>
                        <h2 className="pb-3 text-sm text-gray-500 tracking-[0.10em] font-semibold">{reservation?.reservationNr || reservation?.id ? (<>BOKNING <span className="ml-2">Nr. {reservation?.reservationNr ?? reservation?.id}</span></>) : ("Ny bokning")}</h2>
                                ...
                                <div className='flex items-center gap-6 flex-wrap'>
                                    ...
                                    <div className='ml-20 flex items-center flex-wrap gap-3'>
                                        ...
                                        <ActionButton
                                            ...
                                            accent="yellow"
                                            disabled={!canReturnItems}
                                        />
                                        <ActionButton
                                            label="Checka in"
                                            icon={CheckCircle2}
                                            onClick={handleCheckaIn}
                                            accent="yellow"
                                            disabled={!canCheckIn}
                                        />
                                    </div>
                            ...
                            <div className='grid grid-cols-[320px_auto_250px_200px_auto] gap-15'>
                                ...
                                <span>
                                    {isVehicleEnabledForOffice && (
                                        <span id="driver-info" className="w-40">
                                            <LabeledInput
                                                label="Förare"
                                ...
                                <span>
                                    ...
                                    <LabeledTextArea
                                        ...
                                        label='Notering, extern'
                                        value={reservation?.noteExternal || ''}
                                        onChange={(e) => handleChange('noteExternal', e)}
                                        labelWidth="w-18"
                                        margintop="0"
                                        height='h-16'
                                        placeholder="" />
                                ...
                                <span className="w-52">
                                    ...
                                    <LabeledTextArea
                                        ...
                                        placeholder="" />
                                    <LabeledCheckbox
                                        id="is-ongoing-invoicing"
                                        name="isOngoingInvoicing"
                                        label="Löpande fakturering"
                                        labelWidth="w-28"
                                        labelPosition="left"
                                        checked={Boolean(reservation?.isOngoingInvoicing)}
                                        onChange={(checked) => handleChange('isOngoingInvoicing', checked)}
                                        color="green"
                                        className="mt-2"
                                    />
                                </span>
                                        ...
                                        <div className="grid grid-cols-2 gap-5 w-full">
                                            ...
                                            <div className="rounded-sm border border-gray-200 bg-white px-3 py-2">
                                                ...
                                                <table className="mt-1 w-full text-xs text-gray-700" style={{ fontFamily: "'Neue Haas Unica', 'Helvetica Neue', Arial, sans-serif" }}>
                                                    <tbody>
                                                        {Object.entries(invoicePartyLabels).map(([partyKey, label]) => {
                                                            const amountLeftExVat = amountLeftByParty[partyKey].exVat;
                                                            const amountLeftInclVat = amountLeftByParty[partyKey].inclVat;
                                                            const exVatClassName = amountLeftExVat > 0
                                                                ? 'tabular-nums py-0.5 text-right font-bold text-red-600'
                                                                : 'tabular-nums py-0.5 text-right text-gray-500';
                                                            const inclVatClassName = amountLeftInclVat > 0
                                                                ? 'tabular-nums py-0.5 text-right font-bold text-red-600'
                                                                : 'tabular-nums py-0.5 text-right';

                                                            return (
                                                                <tr key={`left-${partyKey}`} className="align-top">
                                                                    <td className="pr-2 py-0.5">{label}</td>
                                                                    <td className={exVatClassName}>{formatAmount(amountLeftExVat)} ex</td>
                                                                    <td className={inclVatClassName}>{formatAmount(amountLeftInclVat)} ink</td>
                                                                </tr>
                                                            );
                                                        })}
                                                    </tbody>
