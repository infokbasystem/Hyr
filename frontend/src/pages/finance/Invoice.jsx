...
import { ArrowLeft, Ban, Landmark, Printer, Save, Smartphone, Trash2 } from 'lucide-react';
...
import { createTinkPaymentRequest, getTinkPaymentStatus, sendTinkPaymentRequest } from '../../lib/tinkApi';
...
const Invoice = () => {
    ...
    const initialFormOptions = initialSavedInvoiceEntry?.formOptions ?? { currencies: [], paymentMethods: [], systemAccountNumbers: {} };
    ...
    const [swishMobilePhone, setSwishMobilePhone] = useState('');
    const [showTinkModal, setShowTinkModal] = useState(false);
    const [tinkEmail, setTinkEmail] = useState('');
    const [tinkMobilePhone, setTinkMobilePhone] = useState('');
    const [tinkPaymentRequest, setTinkPaymentRequest] = useState(null);
    const [tinkLoading, setTinkLoading] = useState(false);
    const [tinkSending, setTinkSending] = useState(null);
    const [tinkMessage, setTinkMessage] = useState(null);
    ...
    const [canDeleteInvoice, setCanDeleteInvoice] = useState(null);
    ...
    useEffect(() => {
        ...
        getSharedInvoiceRequest('invoice-form-options', async () => {
            ...
            .then((data) => {
                ...
                const systemAccountNumbers = data?.systemAccountNumbers
                    ?? data?.SystemAccountNumbers
                    ?? {};
                ...
                setFormOptions({
                    ...
                    systemAccountNumbers,
                    tinkEnabled: Boolean(data?.tinkEnabled ?? data?.TinkEnabled),
                });
            })
            .catch(() => {
                if (!isActive) {
    ...
    useEffect(() => {
        ...
        if (!invoice?.id || invoice?.invoiceNr == null) {
            setCanDeleteInvoice(null);
            return () => {
                isActive = false;
            };
        }
        ...
        const queryParams = new URLSearchParams({
            SortBy: 'InvoiceNr:desc',
            Page: '1',
            PageSize: '1',
        });

        apiClient.get(`/invoice?${queryParams.toString()}`)
            .then((response) => {
                if (!isActive) {
                    return;
                }

                const highestInvoice = response.data?.data?.[0];
                setCanDeleteInvoice(highestInvoice?.invoiceNr === invoice.invoiceNr);
            })
            .catch(() => {
                if (isActive) {
    ...
    const handleChange = (field, e) => {
        ...
        setInvoice(prev => ({
            ...prev,
            ...(field === 'invoicePayMethod' && {
                accountNr: e === 'CASH'
                    ? formOptions.systemAccountNumbers?.RECEIVABLE_CASH ?? prev?.accountNr ?? null
                    : formOptions.systemAccountNumbers?.RECEIVABLE_INVOICE ?? prev?.accountNr ?? null,
            }),
            [field]: e
        }));
    };
    ...
    const handleCancelInvoice = async () => {
        if (!invoice?.id || invoice.isCancelled) {
            return;
        }

        const cancelledInvoice = { ...invoice, isCancelled: true };
        setInvoice(cancelledInvoice);
        const wasSaved = await submitInvoice(undefined, cancelledInvoice);
        if (wasSaved) {
            setTimeout(handleBackClick, 1000);
        }
    };

    const handleSwishClick = async () => {
        handleChange('invoicePayMethod', 'SWISH');
        setSwishMobilePhone('');
        setShowSwishModal(true);
        ...
        try {
            ...
            setSwishMobilePhone(customer?.mobilePhone ?? '');
        } catch (error) {
            console.error('Error loading customer mobile phone:', error);
        }
    };

    const handleSwishSend = () => {
        setShowSwishModal(false);
        setMessages([{ type: 'success', text: `Swish-betalning skickad till ${swishMobilePhone}` }]);
    };

    const handleTinkClick = async () => {
        if (!invoice?.id) {
            setMessages([{ type: 'error', text: 'Spara fakturan innan du skapar en Tink-betalning' }]);
            return;
        }

        handleChange('invoicePayMethod', 'TINK');
        setTinkEmail('');
        setTinkMobilePhone('');
        setTinkPaymentRequest(null);
        setTinkMessage(null);
        setShowTinkModal(true);
        setTinkLoading(true);

        try {
            const [paymentRequest, customer] = await Promise.all([
                createTinkPaymentRequest(invoice.id),
                invoice.customerId ? getCustomerById(invoice.customerId) : Promise.resolve(null),
            ]);

            setTinkPaymentRequest(paymentRequest);
            setTinkEmail(customer?.email ?? paymentRequest?.customerEmail ?? '');
            setTinkMobilePhone(customer?.mobilePhone ?? paymentRequest?.customerMobilePhone ?? '');
        } catch (error) {
            const errorText = error?.payload?.message ?? error?.message ?? 'Kunde inte skapa Tink-betalning';
            setTinkMessage({ type: 'error', text: errorText });
        } finally {
            setTinkLoading(false);
        }
    };

    const handleTinkSend = async (channel) => {
        if (!tinkPaymentRequest?.id) {
            return;
        }

        const recipient = channel === 'EMAIL' ? tinkEmail : tinkMobilePhone;
        setTinkSending(channel);
        setTinkMessage(null);

        try {
            const updated = await sendTinkPaymentRequest(tinkPaymentRequest.id, channel, recipient);
            setTinkPaymentRequest(updated);
            setTinkMessage({
                type: 'success',
                text: channel === 'EMAIL'
                    ? `Betallänk skickad till ${recipient}`
                    : `Betallänk skickad via SMS till ${recipient}`,
            });
        } catch (error) {
            const errorText = error?.payload?.message ?? error?.message ?? 'Kunde inte skicka betallänken';
            setTinkMessage({ type: 'error', text: errorText });
        } finally {
            setTinkSending(null);
        }
    ...
    const handleTinkCopyLink = async () => {
        if (!tinkPaymentRequest?.linkUrl) {
            return;
        }

        try {
            await navigator.clipboard.writeText(tinkPaymentRequest.linkUrl);
            setTinkMessage({ type: 'success', text: 'Betallänken kopierad' });
        } catch {
            setTinkMessage({ type: 'error', text: 'Kunde inte kopiera betallänken' });
        }
    };

    const handleTinkCheckStatus = async () => {
        if (!tinkPaymentRequest?.id) {
            return;
        }

        setTinkSending('STATUS');
        setTinkMessage(null);

        try {
            const updated = await getTinkPaymentStatus(tinkPaymentRequest.id);
            setTinkPaymentRequest(updated);

            if (updated?.isSettled) {
                setTinkMessage({ type: 'success', text: 'Betalningen är genomförd och fakturan är reglerad' });
                const refreshed = await getInvoiceById(invoice.id, null, { dedupe: false });
                if (refreshed) {
                    setInvoice(refreshed);
                    setOriginalInvoice(JSON.parse(JSON.stringify(refreshed)));
                }
            } else {
                setTinkMessage({ type: 'success', text: updated?.statusMessage || `Status: ${updated?.status || 'okänd'}` });
            }
        } catch (error) {
            const errorText = error?.payload?.message ?? error?.message ?? 'Kunde inte hämta betalstatus';
            setTinkMessage({ type: 'error', text: errorText });
        } finally {
            setTinkSending(null);
        }
    ...
    const handleOpenCustomer = () => {
        ...
        window.open(`/customer/${invoice.customerId}`, '_blank', 'noopener,noreferrer');
    };
    ...
    const submitInvoice = async (e, invoiceOverride = null) => {
        e?.preventDefault?.();
        ...
        const invoiceData = { ...(invoiceOverride ?? invoice) };
    ...
    const deleteInvoice = async () => {
        ...
        try {
            ...
            const errorText = error?.response?.data?.message ?? error?.response?.data;
            if (errorText) {
                setMessages([{ type: 'error', text: errorText || 'Kunde inte ta bort fakturan' }]);
                return;
            }
        ...
        <div className="relative flex flex-col h-full md:px-[clamp(4px,3vw,6vw)]">
            ...
            {showSwishModal && (
                <div className="fixed inset-0 z-50">
                    ...
                    <div className="relative z-50 flex min-h-screen items-start justify-center pt-20" onClick={() => setShowSwishModal(false)}>
                        <div
                        ...
                        >
                            <div className="relative flex items-center justify-center mb-4">
                                ...
                                <button
                                    type="button"
                                    onClick={() => setShowSwishModal(false)}
                            ...
                            <div className="mx-10 mt-7">
                                <LabeledInput
                                    ...
                                    value={swishMobilePhone}
                                    onChange={(value) => setSwishMobilePhone(value)}
                                    margintop="0"
                                />
                                <div className="flex gap-4 mt-6 mb-3 pt-4 justify-end">
                                    ...
                                    <button
                                        type="button"
                                        onClick={handleSwishSend}
                                        disabled={!swishMobilePhone}
            ...
            {showTinkModal && (
                <div className="fixed inset-0 z-50">
                    <div className="absolute inset-0 bg-black/50 z-40" />
                    <div className="relative z-50 flex min-h-screen items-start justify-center pt-20" onClick={() => setShowTinkModal(false)}>
                        <div
                            className="relative bg-white rounded-sm shadow-xl max-w-xl w-full mx-4 p-6"
                            style={{ background: 'rgb(255, 255, 234)' }}
                            onClick={(e) => e.stopPropagation()}
                        >
                            <div className="relative flex items-center justify-center mb-4">
                                ...
                                <button
                                    type="button"
                                    onClick={() => setShowTinkModal(false)}
                                    className="absolute right-0 text-gray-400 hover:text-gray-600 text-l leading-none mb-1"
                                >
                                    ×
                                </button>
                            ...
                            <div className="mx-10 mt-7">
                                {tinkLoading && (
                                    <p className="text-xs text-gray-600">Skapar betalning hos Tink...</p>
                                )}

                                {tinkMessage && (
                                    <div
                                        className={`mb-4 rounded-sm border px-3 py-2 text-xs text-center ${tinkMessage.type === 'error'
                                    ...
                                    <>
                                        <div className="mb-5 flex items-start gap-6">
                                            <div className="flex-1 space-y-1 text-xs text-gray-700">
                                                <div className="flex justify-between">
                                                    ...
                                                    <span>{Number(tinkPaymentRequest.amount || 0).toLocaleString('sv-SE', { minimumFractionDigits: 2 })} {tinkPaymentRequest.currency}</span>
                                                </div>
                                                <div className="flex justify-between">
                                                    <span className="text-gray-500">Status</span>
                                                    <span>{tinkPaymentRequest.status || '-'}</span>
                                                </div>
                                                <div className="pt-2">
                                                    <a
                                                        href={tinkPaymentRequest.linkUrl}
                                                        target="_blank"
                                                        rel="noopener noreferrer"
                                                        className="break-all text-sky-700 underline"
                                                    >
                                                        {tinkPaymentRequest.linkUrl}
                                                    </a>
                                                </div>
                                            </div>
                                            <div className="shrink-0 bg-white p-2">
                                                <QRCodeSVG value={tinkPaymentRequest.linkUrl} size={120} />
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-[1fr_auto] items-center gap-3">
                                            <LabeledInput
                                                ...
                                                labelWidth="w-20"
                                                value={tinkEmail}
                                                onChange={(value) => setTinkEmail(value)}
                                                margintop="0"
                                            />
                                            <button
                                                type="button"
                                                onClick={() => handleTinkSend('EMAIL')}
                                                disabled={!tinkEmail || tinkSending !== null}
                                                className="shadow-md/30 text-xs text-white bg-lime-700 hover:bg-lime-900 px-6 p-[5px] disabled:opacity-50 disabled:cursor-not-allowed"
                                            >
                                                {tinkSending === 'EMAIL' ? 'Skickar...' : 'Skicka e-post'}
                                            </button>

                                            <LabeledInput
                                                label="Mobilnr"
                                                labelWidth="w-20"
                                                value={tinkMobilePhone}
                                                onChange={(value) => setTinkMobilePhone(value)}
                                                margintop="2"
                                            />
                                            <button
                                                type="button"
                                                onClick={() => handleTinkSend('SMS')}
                                                disabled={!tinkMobilePhone || tinkSending !== null}
                                                className="shadow-md/30 text-xs text-white bg-lime-700 hover:bg-lime-900 px-6 p-[5px] disabled:opacity-50 disabled:cursor-not-allowed"
                                            >
                                                {tinkSending === 'SMS' ? 'Skickar...' : 'Skicka SMS'}
                                            </button>
                                        </div>
                                    </>
                                )}
                                ...
                                <div className="flex gap-4 mt-6 mb-3 pt-4 justify-end">
                                    ...
                                    <button
                                        type="button"
                                        onClick={handleTinkCopyLink}
                                        disabled={!tinkPaymentRequest?.linkUrl}
                                        className="shadow-md/30 text-xs text-white bg-sky-700 hover:bg-sky-900 px-6 p-[5px] disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
                                        Kopiera länk
                                    </button>
                                    <button
                                        type="button"
                                        onClick={handleTinkCheckStatus}
                                        disabled={!tinkPaymentRequest?.id || tinkSending !== null}
                                        className="shadow-md/30 text-xs text-white bg-lime-700 hover:bg-lime-900 px-6 p-[5px] disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
                                        {tinkSending === 'STATUS' ? 'Hämtar...' : 'Kontrollera status'}
                                    </button>
                                </div>
                            </div>
                        </div>
            ...
            <div className="mt-1 flex min-h-0 flex-1 flex-col">
                ...
                <div className="grid min-h-0 flex-1 gap-6 lg:grid-cols-[280px_minmax(0,1fr)] lg:gap-8 lg:items-start">
                        ...
                        <aside className="lg:sticky lg:top-[calc(52px+72px+1rem)] lg:z-10 lg:self-start">
                            <div className="space-y-4 pr-0 pb-4 ml-2">
                                ...
                                <div className="space-y-2 text-xs text-gray-500" style={{ fontFamily: "'Neue Haas Unica', 'Helvetica Neue', Arial, sans-serif" }}>
                                    ...
                                    {renderMetaRow('Ändrad:', invoice?.modifiedDate, invoice?.modifiedByUserName)}
                    ...
                    <div className='flex-grow lg:pl-2 pb-10 mr-20'>
                        <h2 className="flex flex-row items-center pb-3 text-sm text-gray-500 tracking-[0.10em] font-semibold">
                            {invoice?.id ? (
                                <>
                                    FAKTURA <span className="ml-2">Nr. {invoice.invoiceNr}</span>
                                    {invoice.isCancelled && (
                                        <div className='mb-[1px]'> 
                                            <span className="ml-6 inline-flex rounded-full border border-rose-300 bg-rose-100 px-2 py-0.5 text-tiny tracking-wider text-rose-700">
                                                Makulerad
                                            </span>
                                        </div>
                                    )}
                        ...
                        <form onSubmit={submitInvoice} autoComplete="off">
                            <div className="flex justify-between w-full mb-6">
                                ...
                                <div className='flex items-center gap-5'>
                                    {invoice?.id != 0 && !invoice?.isCancelled && canDeleteInvoice === true && (
                                        <ActionButton
                                            label="Radera"
                                            icon={Trash2}
                                    ...
                                    {invoice?.id != 0 && !invoice?.isCancelled && canDeleteInvoice === false && (
                                        <ActionButton
                                            label="Makulera"
                                            icon={Ban}
                                ...
                                <div className="grid gap-15 xl:grid-cols-[280px_360px_300px_auto]">
                                    <div className="">
                                        <SearchCustomer
                                            ...
                                            onOpenCustomer={invoice?.customerId ? handleOpenCustomer : undefined}
                                            width="w-full"
                                            className="min-w-0"
