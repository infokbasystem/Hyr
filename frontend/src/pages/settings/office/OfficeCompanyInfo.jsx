...
import { getCompanyInfo, updateCompanyInfo, uploadCompanyLogo, deleteCompanyLogo } from '../../../lib/officeApi'
...
export default function OfficeCompanyInfo() {
    ...
    async function handleLogoFileChange(event) {
        ...
        if (!ALLOWED_LOGO_TYPES.includes(file.type)) {
            setMessages([{ type: 'error', text: 'Filtypen stöds inte. Tillåtna format: PNG, JPEG, GIF, WEBP, SVG.' }])
            return
        }

        if (file.size > MAX_LOGO_SIZE_BYTES) {
            setMessages([{ type: 'error', text: 'Filen är för stor. Max storlek är 2 MB.' }])
            return
        }
        ...
        try {
            const savedOffice = toOfficeState(await uploadCompanyLogo(file))
            setOffice(savedOffice)
            setOriginalOffice(cloneOffice(savedOffice))
            setMessages([{ type: 'success', text: 'Logotyp uppladdad.' }])
        } catch (error) {
            const errorText = error?.payload?.message ?? error?.message ?? 'Kunde inte ladda upp logotyp.'
            setMessages([{ type: 'error', text: errorText }])
        } finally {
    ...
    async function handleRemoveLogo() {
        ...
        try {
            const savedOffice = toOfficeState(await deleteCompanyLogo())
            setOffice(savedOffice)
            setOriginalOffice(cloneOffice(savedOffice))
            setMessages([{ type: 'success', text: 'Logotyp borttagen.' }])
        } catch (error) {
            const errorText = error?.payload?.message ?? error?.message ?? 'Kunde inte ta bort logotyp.'
            setMessages([{ type: 'error', text: errorText }])
        } finally {
            setIsUploadingLogo(false)
        }
                        ...
                        <div className="max-w-[500px]">
                            ...
                            <div>
                                ...
                                <LabeledInput
                                    ...
                                    label="Självriskred. kr/dag"
                                    labelWidth="w-20"
                                    inputWidth="w-full"
                                    margintop="0"
                                    type="number"
                                    value={office?.deductibleReductionCostPerDay ?? ''}
                                    onChange={(value) => updateField('deductibleReductionCostPerDay', value ?? '')}
                                />
                            </div>

                            <div className="mt-10">
                                <h2 className="mb-3 text-xs uppercase tracking-[0.12em] text-gray-600">Logotyp</h2>
                                <div className="flex items-center gap-6">
                                    <div className="flex h-30 w-80 items-center justify-center rounded border border-dashed border-gray-300 bg-gray-50">
                                        {office?.logoDataUrl ? (
                                            <img src={office.logoDataUrl} alt="Företagslogotyp" className="max-h-full max-w-full object-contain" />
                                        ) : (
                                            <span className="text-[10px] uppercase tracking-[0.1em] text-gray-400">Ingen logotyp</span>
                                        )}
                                    </div>
                                </div>
