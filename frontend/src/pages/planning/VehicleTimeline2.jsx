...
const VehicleTimelineBar = memo(function VehicleTimelineBar({
    ...
    return (
        <div
            ...
            style={{
                position: "absolute",
                left: `calc(${booking.start * dayW}px - var(--timeline-scroll-x))`,
                top: 0,
    ...
    return (
        <div className="px-4" style={{ position: "relative" }}>
                        ...
                        <div style={{ height: headerH, position: "sticky", top: 0, zIndex: 20, width: "100%", overflow: "hidden", borderBottom: `1px solid ${T.border}`, background: `url(${bg})` }}>
                            ...
                            {showMonthRow && (() => {
                                ...
                                return spans.map((span, si) => {
                                    ...
                                    return (
                                        <div key={si} style={{
                                            position: "absolute", left: `calc(${span.startPx}px - var(--timeline-scroll-x))`, top: 0,
                            ...
                            {/* Day columns */}
                            {Array.from({ length: totalTimelineDays }, (_, i) => {
                                const dayIdx = i - populatedPastDays;
                                const label = getDayLabel(dayIdx, startDate);
                                return (
                                    <div key={dayIdx} style={{ position: "absolute", left: `calc(${dayIdx * dayW}px - var(--timeline-scroll-x))`, top: showMonthRow ? MONTH_ROW_H : 0, width: dayW, height: `calc(100% - ${showMonthRow ? MONTH_ROW_H : 0}px)`, borderRight: `1px solid ${T.border}`, background: label.isToday ? T.todayBg : label.isWeekend ? T.weekendBg : "white", display: "flex", flexDirection: "column" }}>
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {Array.from({ length: totalTimelineDays }, (_, i) => {
                                        ...
                                        const label = getDayLabel(dayIdx, startDate);
                                        return (
                                            <div key={i} style={{ position: "absolute", left: `calc(${dayIdx * dayW}px - var(--timeline-scroll-x))`, top: 0, bottom: 0, width: dayW, borderRight: `1px solid ${T.gridLine}`, background: label.isToday ? T.todayBg : label.isWeekend ? T.weekendRow : "transparent", pointerEvents: "none" }}>
                                                {hourTicks.map(h => (
            ...
            <div ref={ghostRef} style={{
                position: "fixed", left: (ghost?.x ?? 0) - (ghost?.clickOffsetX ?? 0), top: (ghost?.y ?? 0) - (ghost?.clickOffsetY ?? 0),
            ...
            <div ref={ghostTextRef} style={{
                position: "fixed",
                left: `${typeof window !== "undefined" ? clamp(ghost?.x ?? 0, 170, window.innerWidth - 170) : (ghost?.x ?? 0)}px`,
                top: `${Math.max(8, (ghost?.y ?? 0) - 34)}px`,
